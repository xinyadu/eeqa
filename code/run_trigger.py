...
def read_ace_examples(input_file, tokenizer, category_vocab, is_training):
...
def evaluate(args, eval_examples, category_vocab, model, device, eval_dataloader, pred_only=False):
    ...
    # get predictions
    pred_triggers = dict()
    for idx, (sentence_id, input_ids, input_mask, labels) in enumerate(eval_dataloader):
...
def main(args):
    ...
    if args.do_train or (not args.eval_test):
        eval_examples, eval_features = read_ace_examples(input_file=args.dev_file, tokenizer=tokenizer, category_vocab=category_vocab, is_training=False)
        if args.add_lstm:
        ...
        eval_data = TensorDataset(all_sentence_id, all_input_ids, all_input_mask, all_labels)
        eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size)

    if args.do_train:
        _, train_features = read_ace_examples(input_file=args.train_file, tokenizer=tokenizer, category_vocab=category_vocab, is_training=True)
    ...
    if args.do_eval:
        if args.eval_test:
            eval_examples, eval_features = read_ace_examples(input_file=args.test_file, tokenizer=tokenizer, category_vocab=category_vocab, is_training=False)
            if args.add_lstm:
                eval_features = sorted(eval_features, key=lambda f: np.sum(f.input_mask), reverse=True)
            logger.info("***** Test *****")
            logger.info("  Num orig examples = %d", len(eval_features))
